...
const DocumentList = forwardRef<DocumentListRef, DocumentListProps>(
    ({ workspaceId, onDocumentChange, onDocumentCountChange, disableActions = false }, ref) => {
        ...
        // Get real-time document status updates from Redux
        const documentStatusUpdates = useSelector((state: RootState) => state.status.documents);

        const loadDocuments = useCallback(async () => {
            try {
                setLoading(true);
                setError('');
                const response = await apiService.listDocuments(workspaceId);
        ...
        const getDocumentStatus = (doc: DocumentWithStatus): DocumentStatus => {
            // Check for real-time status update first
            const update = documentStatusUpdates[doc.id];
            if (update) {
                return update.status;
            }
            // Infer status from document data
            if (doc.status) return doc.status;
            if (doc.chunk_count && doc.chunk_count > 0) return 'ready';
            return 'pending';
        };
                ...
                {documents.length === 0 ? (
                    ...
                    <div className="divide-y divide-gray-200 dark:divide-gray-700">
                        <div className="px-4 py-2 bg-gray-100 dark:bg-gray-800 flex items-center text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase">
                            ...
                            <div className="w-24 text-right">Chunks</div>
                            <div className="w-32 text-right">Uploaded</div>
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="w-24 flex justify-center">
                                        <StatusBadge
                                            status={status}
                                            message={doc.status_error || undefined}
                                            size="sm"
                                        />
                                    </div>
