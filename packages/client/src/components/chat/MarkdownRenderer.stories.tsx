...
const MockMarkdownRenderer = ({ content, isUser = false }: { content: string; isUser?: boolean }) => {
  ...
  const renderContent = (text: string) => {
    ...
    return text
      ...
      .map((line, index) => {
        ...
        if (line.startsWith('```')) {
          return (
            <pre key={index} className="bg-gray-100 dark:bg-gray-800 p-3 rounded-md overflow-x-auto">
...
const meta: Meta<typeof MockMarkdownRenderer> = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: 'Markdown content renderer with syntax highlighting. (Mock implementation for Storybook)',
      },
    },
  },
  tags: ['autodocs'],
