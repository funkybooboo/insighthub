...
class WikipediaWorker(BaseWorker):
    ...
    def process_event(self, event_data: dict[str, Any]) -> None:
        ...
        try:
            self._publish_status(
                workspace_id, query, "fetching", "Searching Wikipedia...", []
            )
            pages = self.fetcher.search_and_fetch(query)

            if not pages:
                self._publish_status(
                    workspace_id, query, "failed", "No Wikipedia pages found", []
                )
                return
            ...
            document_ids = []
            for page in pages:
                file_path, doc_id = self._create_document_from_page(
                    workspace_id, user_id, page
                )
                document_ids.append(doc_id)
                self._publish_document_uploaded_event(
                    workspace_id, user_id, doc_id, file_path, page
                )
            ...
            self._publish_status(
                ...
                query,
                "completed",
                f"Created {len(document_ids)} documents",
                document_ids,
            )
            logger.info(
            ...
            self._publish_status(
                workspace_id, query, "failed", f"Failed to fetch content: {str(e)}", []
            )
            raise

    def _create_document_from_page(
        self, workspace_id: str, user_id: str, page: dict[str, Any]
    ) -> Tuple[str, str]:
    ...
    def _publish_document_uploaded_event(
        self,
        workspace_id: str,
        user_id: str,
        document_id: str,
