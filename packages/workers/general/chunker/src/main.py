"""
Chunker Worker - Document chunking for embeddings.
...
from .chunker_worker import create_chunker_worker

logger = logging.getLogger(__name__)


async def main() -> None:
    """Main entry point for the chunker worker."""
    # Setup logging
    logging.basicConfig(
    ...
    logger.info("Starting Chunker Worker")
    ...
    # Create worker
    worker = create_chunker_worker(consumer, publisher, config)

    # Connect and declare queue
    consumer.connect()
    consumer.declare_queue("chunker_queue", "document.parsed")
    worker.set_queue_name("chunker_queue")
