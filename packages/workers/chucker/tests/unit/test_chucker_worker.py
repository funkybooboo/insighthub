...
@pytest.fixture
def chucker_worker():
    with patch("src.main.PostgresConnection"), patch("src.main.create_chunker"):
...
def test_process_event_success(chucker_worker: ChuckerWorker, mocker):
    ...
    mocker.patch.object(
        chucker_worker,
        "_get_parsed_text",
        return_value="This is a test text for chunking.",
    )
    mocker.patch.object(
        chucker_worker, "_store_chunks", return_value=["chunk1", "chunk2"]
    )
    ...
    chucker_worker._update_document_status.assert_any_call(
        "doc123", "chunked", {"chunk_count": 2}
    )
    ...
    assert (
        call_args.kwargs["event"]["document_id"] == expected_chunked_event.document_id
    )
    assert (
        call_args.kwargs["event"]["chunk_count"] == expected_chunked_event.chunk_count
    )
