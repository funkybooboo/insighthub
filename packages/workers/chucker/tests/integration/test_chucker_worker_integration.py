...
def test_chucker_worker_integration(postgres_container, rabbitmq_container):
    ...
    cursor.execute("INSERT INTO documents (id, parsed_text) VALUES (%s, %s)", (1, "This is a long text to be chunked into several pieces."))
    pg_conn.commit()
    cursor.close()
    ...
    # 4. Consume result
    result = None
    for method_frame, properties, body in rabbit_channel.consume('chucker.document.parsed', inactivity_timeout=5):
        if body:
