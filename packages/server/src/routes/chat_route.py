...
def chat() -> tuple[Response, int]:
    ...
    try:
        ...
        if session_id:
            session = ctx.chat_service.get_session_by_id(int(session_id))
            if not session:
                return jsonify({"error": "Session not found"}), 404
        else:
            # Create new session with first message as title
            session = ctx.chat_service.create_session(
                user_id=user.id, first_message=user_message, rag_type=rag_type
            )

        # Store user message
        ctx.chat_service.create_message(session_id=session.id, role="user", content=user_message)
        ...
        ctx.chat_service.create_message(
...
def list_sessions() -> tuple[Response, int]:
    ...
    try:
        ...
        user = ctx.user_service.get_or_create_default_user()
        sessions = ctx.chat_service.list_user_sessions(user.id)
...
def get_session_messages(session_id: int) -> tuple[Response, int]:
    ...
    try:
        ...
        messages = ctx.chat_service.list_session_messages(session_id)
