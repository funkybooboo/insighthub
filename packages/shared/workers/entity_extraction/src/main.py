"""Entity extraction worker main entry point."""
...
from .entity_extraction_worker import create_entity_extraction_worker


def main() -> None:
    """Main entry point for entity extraction worker."""
    logging.basicConfig(
        level=logging.INFO,
    ...
    try:
        ...
        worker = create_entity_extraction_worker(consumer, publisher, config)
        worker.run()

    except KeyboardInterrupt:
        logger.info("Entity extraction worker stopped by user")
    except Exception as e:
        logger.error(f"Entity extraction worker failed: {e}")
        sys.exit(1)
