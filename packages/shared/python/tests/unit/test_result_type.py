...
class TestOkContract:
    ...
    def test_ok_with_different_types(self) -> None:
    ...
    def test_ok_map_preserves_ok_type(self) -> None:
...
class TestErrContract:
    ...
    def test_err_unwrap_or_returns_default(self) -> None:
        ...
        result2: Result[str, str] = Err("failed")
        assert result2.unwrap_or("default") == "default"
    ...
    def test_err_with_different_error_types(self) -> None:
        ...
        # Dictionary error
        result_dict: Err[dict[str, str]] = Err({"code": "INVALID", "message": "Bad request"})
...
class TestResultTypeUsage:
    ...
    def test_result_with_is_ok_pattern(self) -> None:
    ...
    def test_result_with_is_err_pattern(self) -> None:
    ...
    def test_result_chaining_with_map(self) -> None:
    ...
    def test_result_early_return_pattern(self) -> None:
        """Test early return pattern with Result."""

        def fetch_user(user_id: int) -> Result[dict[str, str | int], str]:
            if user_id <= 0:
    ...
    def test_result_unwrap_or_default_pattern(self) -> None:
    ...
    def test_result_replaces_none_pattern(self) -> None:
        """
        ...
        def find_old(items: list[int], target: int) -> int | None:
            try:
        ...
        def find_new(items: list[int], target: int) -> Result[int, str]:
            try:
...
class TestResultTypeContract:
    ...
    def test_result_is_either_ok_or_err(self) -> None:
    ...
    def test_result_types_are_distinct(self) -> None:
        """Test that Ok and Err are distinct types."""
        ok_result = Ok(42)
        err_result = Err("error")

        assert not isinstance(ok_result, type(err_result))
        assert isinstance(ok_result, Ok)
        assert isinstance(err_result, Err)

    def test_result_preserves_type_information(self) -> None:
