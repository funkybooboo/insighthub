...
def test_rag_type_default_ported():
    cfg = RagConfig()
    assert cfg.rag_type == "vector"


def test_chunking_strategy_default_ported():
    cfg = RagConfig()
    assert cfg.chunking_strategy == "sentence"


def test_chunk_size_is_positive_ported():
    cfg = RagConfig()
    assert isinstance(cfg.chunk_size, int)
    assert cfg.chunk_size > 0


def test_chunk_overlap_is_valid_ported():
    cfg = RagConfig()
...
def test_rag_top_k_is_positive_ported():
    cfg = RagConfig()
